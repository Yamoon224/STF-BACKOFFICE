        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {auditLogs.map((log, i) => (
                <tr key={i}>
                  <td className="py-4 font-mono text-xs text-slate-500">{log.time}</td>
                  <td className="py-4 font-medium text-stf-navy">{log.actor}</td>
                  <td className="py-4 text-slate-500">{log.action}</td>
