  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {filters.map((f) => (
          <span key={f} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-500">
            {f} ▾
          </span>
        ))}
      </div>
      ...
      <Card title="Exports disponibles">
        <div className="space-y-3">
          {reports.map((r) => (
            <div key={r.name} className="flex items-center justify-between rounded-xl border border-slate-100 p-4">
              <div>
                <p className="text-sm font-semibold text-stf-navy">{r.name}</p>
                <p className="text-xs text-slate-400">{r.program}</p>
              </div>
              <button className="rounded-full border border-stf-blue px-4 py-2 text-xs font-semibold text-stf-blue hover:bg-stf-blue-light">
                Exporter {r.format}
              </button>
            </div>
          ))}
